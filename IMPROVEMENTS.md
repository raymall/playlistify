...
## The floating Supabase CLI has no darwin-arm64 binary

- **In plain terms:** the documented way to run migrations — `npx supabase` —
  now fails outright on this Mac, because the newest CLI release ships no binary
  for the machine. Pin a working version so the database workflow is
  reproducible instead of depending on whatever npx resolves that day.
- **Severity:** High — every step of the database sequence in
  `.claude/rules/database.md` goes through `npx supabase`, and
  `scripts/gen-types.mjs` is hard-wired to it, so a bad upstream release blocks
  all schema work with an error that looks like a machine problem.
- **Issue:** `npx supabase` resolves to the floating latest (2.114.0 on
  2026-08-13), which throws
  `No matching Supabase CLI binary package found for darwin-arm64` before
  running anything. 2.113.0 works and is present in the npx cache. The
  2026-08-13 vocabulary migration was applied with an explicit
  `npx supabase@2.113.0`, and `types.ts` was regenerated by calling the cached
  binary directly, bypassing `scripts/gen-types.mjs`.
- **Why fix:** add the CLI as a pinned devDependency so `npx supabase` resolves
  from `node_modules/.bin`, then drop the version from the rule file. That also
  removes the silent dependency on one developer's npx cache.
